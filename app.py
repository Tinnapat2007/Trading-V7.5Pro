...
def build_trade_call(asset: Asset, state: dict, tv: dict | None = None, data_source: str = "N/A") -> dict:
    ...
    state["direction_bias"] = safe_float(state.get("direction_bias")) + state["tv_score"]
...
st.caption("Data: Binance Spot สำหรับ Crypto, Yahoo Finance fallback/Forex/Gold, TradingView consensus เป็นคะแนนยืนยันเสริม")
...
if (st.session_state.run_once or st.session_state.auto_mode) and assets:
    ...
    for index, asset in enumerate(assets):
        ...
        call = build_trade_call(asset, state, tv=tv, data_source=data_source)
        calls.append(call)
...
<div class="trade-card">
  ...
  <div class="trade-meta">
    ...
    TV {best_state["tv_recommend"]} · สภาวะ: {best["run_state"]}<br>
